...
const subir = () => {
    ...
    const archivos = document.getElementById("photo").files;
    if (!validateFotos(archivos)){
        errores += "Se deben subir entre 1 y 3 fotos. \n";
        puede = false;
    }
    ...
    if (!validateProducerName(nombreProd)) {
        errores += "El nombre debe tener un largo mínimo 3 y máximo 80. \n";
        puede = false;
    }

    const emailProd = document.getElementById("email").value;
    if (!validateEmail(emailProd)) {
